...
#rar x -p'meituge' $folders
7z x -p'meituge' $folders
if [ $? != 0 ]
then
    echo -e "\e[31mfailed!!!!!!!!!!!!!!!!!!!!\e[0m"
    exit 1
...
for folders in `find . -type f -name "*.mtg"`
#FILES=*
#for folders in $FILES
do 
echo $folders
...
for folders in `find . -type f -name "*.mtuge"`
#FILES=*
#for folders in $FILES
do 
echo $folders
...
do 
echo $folders
unzip -O cp936 -P 'meituge' $folders
if [ $? != 0 ]
then
    echo -e "\e[31mfailed!!!!!!!!!!!!!!!!!!!!\e[0m"
    exit 1
...
#7z -scs936 l $folders
#7za -p'meituge' -mcp=936 -y x $folders   # 7z -scs=/-mcp=, 这种回答就纯瞎扯了, 这是生成时候用的, 解压时候无效.
